...
export const ExperienceSection: React.FC<ExperienceSectionProps> = ({ language = 'en' }) => {
  ...
  return (
    <section id="experience" className="section-ambient-container ambient-theme-indigo py-16 sm:py-20 border-y border-[#fecdd3]/70 dark:border-white/[0.08] overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1160px] mx-auto px-5 sm:px-6">
        <SectionHeader
          label={t?.label || "06 · EXPERIENCE"}
          heading={t?.heading || "Enterprise Track Record"}
        />

        <div className="card-level-1 p-6 sm:p-8 bg-white/95 dark:bg-[#141924]/90 border border-[#fecdd3] dark:border-white/10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-[#fecdd3]/60 dark:border-[#262c36]">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="font-mono text-xs font-bold text-[#e11d48] dark:text-[#fbbf24]">VOIS</span>
                <span className="text-xs font-mono text-[#a06b7a] dark:text-[#8b93a1]">· Enterprise Telecommunications Analytics</span>
              </div>
              <h3 className="font-display font-bold text-2xl text-[#2d151c] dark:text-white">
                Data Analyst Intern
              </h3>
              <p className="text-sm text-[#5e3240] dark:text-[#9ea7b4]">
                Vodafone Intelligent Solutions (VOIS) — Remote
              </p>
            </div>

            <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg card-level-3 font-mono text-xs text-[#2d151c] dark:text-white self-start md:self-auto bg-rose-500/5 dark:bg-white/5 border border-[#fecdd3]/60 dark:border-white/10 shadow-2xs">
              <Clock className="w-3.5 h-3.5 text-[#e11d48] dark:text-[#fbbf24]" />
              <span>Sep 2025 – Oct 2025</span>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 my-6">
            <div className="card-level-2 p-4 bg-white/80 dark:bg-white/[0.03] border border-[#fecdd3]/60 dark:border-white/10 group hover:-translate-y-0.5 transition-transform">
              <div className="font-mono font-bold text-xl text-emerald-600 dark:text-emerald-400">
                +35%
              </div>
              <div className="font-mono text-xs text-[#2d151c] dark:text-white font-semibold mt-0.5">
                {language === 'de' ? "Berichtsgenauigkeit" : language === 'fr' ? "Précision des Rapports" : language === 'hi' ? "रिपोर्टिंग सटीकता" : "Reporting Accuracy"}
              </div>
              <div className="text-xs text-[#5e3240] dark:text-[#8b93a1] mt-1 font-normal">
            ...
            <div className="card-level-2 p-4 bg-white/80 dark:bg-white/[0.03] border border-[#fecdd3]/60 dark:border-white/10 group hover:-translate-y-0.5 transition-transform">
              <div className="font-mono font-bold text-xl text-[#e11d48] dark:text-[#fbbf24]">
                -40%
              </div>
              <div className="font-mono text-xs text-[#2d151c] dark:text-white font-semibold mt-0.5">
          ...
          <div className="space-y-3 text-sm text-[#5e3240] dark:text-[#9ea7b4]">
            <div className="flex items-start gap-3">
              <span className="w-1.5 h-1.5 rounded-full bg-[#e11d48] dark:bg-[#fbbf24] mt-2 flex-shrink-0" />
              <p>
                {language === 'de'
